...
public class BloomFilter {
	...
	public BloomFilter(){
		for(int i = 0; i < seeds.length; i++){
			func[i] = new Hash(BIT_SIZE, seeds[i]);//生成8个指纹信息，范围是BIT_SIZE
		}
	}
	...
	{  
		...
		if(value != null){
			for(Hash f : func){
				//将value生成8个指纹信息,并将其隐射到BIT_SIZE中，置为1。即生成该URL的指纹信息
				bits.set(f.hash(value), true);//将指定索引处的位设置为指定的值
				System.out.println("set hash is:  "+f.hash(value));
			}     
		}
		System.out.println("================================");
	...
	public static class Hash{
		...
		public int hash(String value){
			...
			for(int i = 0; i < len; i++){
				System.out.println("value  :"+value.charAt(i));
