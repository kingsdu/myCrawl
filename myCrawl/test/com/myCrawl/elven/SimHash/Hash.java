 ...
 * <p>
 * Motivation: a need for fast, well distributed, cross-platform hash functions.
...
public class Hash {
	/**
	 * A Java implementation of hashword from lookup3.c by Bob Jenkins (<a
	 * href="http://burtleburtle.net/bob/c/lookup3.c">original source</a>).
	 * 
	 * @param k
	 *            the key to hash
	 * @param offset
	 *            offset of the start of the key
	 * @param length
	 *            length of the key
	 * @param initval
	 *            initial value to fold into the hash
	 ...
	 * lookup3(k,offset,length,initval-(length<<2))} and {@code
	 * lookup3ycs(k,offset,length,initval+(length<<2)) ==
	...
	/**
	 * <p>
	 * The hash value of a character sequence is defined to be the hash of it's
	 * unicode code points, according to
	 * {@link #lookup3ycs(int[] k, int offset, int length, int initval)}
	 * </p>
	 * <p>
	 * If you know the number of code points in the {@code CharSequence}, you
	 * can generate the same hash as the original lookup3 via {@code
	 * lookup3ycs(s, start, end, initval+(numCodePoints<<2))}
	 */
